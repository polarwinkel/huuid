...
def __int32string(num):
    ...
    letters = ['b','d','f','g','h','j','k','l','m','n','p','r','s','t',
            'v','z','B','C','D','F','G','H','J','K','L','M','N','P','R',
            'S','T','V','Z'] # 33 options to start with (C is necessary to achieve 32bit)
    vowels = ['a','e','i','o','u'] # 5 vowels
    consonants = ['b','d','f','g','h','j','k','l','m','n','p','r','s',
            't','v','z'] # 16 phonetic-qunique consonants
    string = ''
    for i in range(9):
        if i == 0:
            # first char can be capital or not + 'C' => 33 options
            j = num % 33
            num = num // 33
            string = string+letters[j]
        elif i % 3 == 1:
            # vowel for 2nd, 5th and 8th char => 5 options
            j = num % 5
            num = num // 5
            string = string+vowels[j]
        else:
            # non-capital consonant for others => 16 options
            j = num % 16
            num = num // 16
            string = string+consonants[j]
    # 33*5*16*16*5*16*16*5*16 options > 2^32 (32bit)
    return string
...
def __str2int32(s):
    ...
    consonants = ['b','d','f','g','h','j','k','l','m','n','p','r','s',
            't','v','z'] # 16 phonetic-qunique consonants
    out = consonants.index(s[8])        # tanslate first char to int
    out = vowels.index(s[7])+out*5      # append 2nd char (5 options) to empty afte *5
    out = consonants.index(s[6])+out*16 # append next 16 options (4bit => *16)
...
def uuid2human(uid, depth=128):
    ...
    ustr = str(uid.hex) # read uid as number
    # convert to huuid-string depending on length:
    if depth == 32:
...
def human2uuid(s):
    ...
    ss = s.split('-') # split to 32bit-blocks ("words")
    out = ''
    for s in ss:
        if len(s) != 9:
            raise ValueError('not a huuid: all blocks must be composed of nine chars')
        int32 = __str2int32(s) # convert huuid-block to int
        out = out+f'0x{int32:08X}'[2:] # convert to hex
    if len(out) == 32:
        # return as UUID-object if possible
